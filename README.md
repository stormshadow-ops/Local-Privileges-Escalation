...
## About
This is just a Bash PoC script, that automates the exploitation steps.
...
Default credentials are 'Z3R0:Z3R0'
If the exploit ran successfully, then you can login using 'su - Z3R0'
...
- The timing to terminate the connection is calculated as the time required to send the dbus-message.
...
- After the time (`$t`) is calculated, the request to insert the `$username` `Z3R0` to the target is repeated 20 times. ```bash
...
- If the user insertion is succesful, [found using `id Z3R0`], then a password hash is generated [using ```bash openssl passwd -5 `echo -n $password` ``` ]. (`$password=Z3R0ftw`)
